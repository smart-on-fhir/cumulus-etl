...
class Scrubber:
    ...
    def _check_system(self, key: str, value: Any) -> Any:
        ...
        if isinstance(value, dict) and "system" in value:
            ...
            code_ok, display_ok = self._is_code_allowed(system, code, display)
    ...
    @staticmethod
    def _is_code_allowed(system: str, code: str | None, display: str | None) -> tuple[bool, bool]:
        # OK, this method is primarily to check on an Epic customer extension point system.
        ...
        # And if code isn't defined for some reason, but there's a display? Don't trust it.
        if code == "0" and display:
            return True, False

        # Epic uses numeric codes for its internal code systems.
        # Non-numeric codes are likely custom.
        # To confirm: is that a guarantee in Epic's valueset editor? i.e. should we be worried
        # about the possibility of a non-zero numeric code with a custom display?
        try:
            int(code)
        except ValueError:
            return False, False
