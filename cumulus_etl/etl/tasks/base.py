...
class EtlTask:
    ...
    def _update_completion_table(self) -> None:
        # FIXME: what about empty sets - do we assume the export gave 0 results or skip it?
        ...
        group = self.task_config.export_group_name
        batch = formats.Batch(
            rows=[
                {
                    "table": output.get_name(self),
                    "group": group,
                    "export_time": self.task_config.export_datetime,
                }
                for output in self.outputs
                if not output.get_name(self).startswith("etl__")
            ],
            schema=self.get_completion_schema(),
        )

        # Write it out
        formatter = self.task_config.create_formatter(
            dbname="etl__completion",
            uniqueness_fields={"table", "group"},
        )
    ...
    def _get_formatter(self, table_index: int) -> formats.Format:
        ...
        if self.formatters[table_index] is None:
            ...
            self.formatters[table_index] = self.task_config.create_formatter(
                ...
                resource_type=table_info.get_resource_type(self),
    ...
    def _write_one_table_batch(self, rows: list[dict], table_index: int, batch_index: int) -> bool:
        ...
        batch = self.make_batch_from_rows(formatter.resource_type, rows, groups=groups)
    ...
    @staticmethod
    def get_completion_schema(*args, **kwargs) -> pyarrow.Schema | None:
        """Returns a schema for the etl__completion table"""
        del args, kwargs
        return pyarrow.schema(
            [
                pyarrow.field("id", pyarrow.string()),
                pyarrow.field("table", pyarrow.string()),
                pyarrow.field("group", pyarrow.string()),
                pyarrow.field("export_time", pyarrow.timestamp("us", "UTC")),
            ]
        )
