...
def sql_visit() -> str:
    ...
    cols = f"ENCOUNTER_NUM, PATIENT_NUM, LOCATION_CD, INOUT_CD, LOCATION_CD, {cols_dates}"
...
def sql_observation_fact() -> str:
    ...
    cols_obs_fact = f"CONCEPT_CD, INSTANCE_NUM, {import_date}, TVAL_CHAR, VALTYPE_CD, VALUEFLAG_CD, OBSERVATION_BLOB"
...
def to_char(column:str, format='YYYY-MM-DD') -> str:
    return f"to_char({column}, '{format}')"
...
def count_by_date(column:str, column_alias:str, count='*', count_alias='cnt', frmt='YYYY-MM-DD') -> str:
    _count = f'count({count}) as {count_alias}'
    _group = format_date(column, column_alias, frmt)
    return _count + ',' + _group
