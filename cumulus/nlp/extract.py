...
async def ctakes_extract_with_retries(
    sentence: str,
    ...
    *,
    ...
    """
    Extract from cTAKES but retry a few times.

    cTAKES has an occasional habit of giving a 500 error if its temporarily overloaded or confused.
    This gives us a little bit of robustness against that
    """
    for i in range(max(0, attempts - 1)):
        try:
            return await ctakesclient.client.extract(sentence, client=client)
        except httpx.HTTPStatusError:
            print("MIKE: got http error! attempt:", i + 1)
            pass
    return await ctakesclient.client.extract(sentence, client=client)  # final try will raise like normal
...
async def extract(
    ...
    try:
        ...
        result = await ctakes_extract_with_retries(sentence, client)
