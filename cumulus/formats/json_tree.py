...
class JsonTreeFormat(store.Format):
    ...
    def _write_records(self, job, df: pandas.DataFrame, processor: Callable[[pandas.Series], None]):
    ...
    def _write_patient(self, patient: pandas.Series) -> None:
        mrn = patient.id
        path = os.path.join(self._dir_output_patient(mrn), "fhir_patient.json")
    ...
    def _write_encounter(self, encounter: pandas.Series) -> None:
        ...
        enc = encounter.id
        path = os.path.join(self._dir_output_encounter(mrn, enc), "fhir_encounter.json")
    ...
    def _write_lab(self, lab: pandas.Series) -> None:
        ...
        path = os.path.join(self._dir_output_encounter(mrn, enc), f"fhir_lab_{lab.id}.json")
    ...
    def _write_condition(self, condition: pandas.Series) -> None:
        ...
        path = os.path.join(self._dir_output_encounter(mrn, enc), f"fhir_condition_{condition.id}.json")
    ...
    def _write_docref(self, docref: pandas.Series) -> None:
        ...
        path = os.path.join(self._dir_output_encounter(mrn, enc), f"fhir_docref_{docref.id}.json")
