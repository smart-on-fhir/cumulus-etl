...
from ctakes.labelstudio import DocAnnot, find_by_name
...
class TestLabelStudio(unittest.TestCase):
    ...
    def ignore_test_select_cohort(self, dir_processed='/opt/i2b2/processed', cohort_size=20):
        ...
        for dirpath, dirs, files in os.walk(dir_processed):
            for filename in files:
                ...
                path_ctakes = os.path.join(dirpath, filename)

                if 'ctakes_' in path_ctakes:
                    if os.path.exists(path_ctakes):
                                ...
                                for select_cui in filter_cui.keys():
                                    ...
                                    if len(cohort[select_cui]) <= cohort_size:
                                        ...
                                        if select_cui in overlap:
                                            physician_note = store.read_text(path_note)

                                            labelstudio = DocAnnot(physician_note=physician_note,
